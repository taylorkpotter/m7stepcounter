...
@interface BounceMenuController () <ADBannerViewDelegate>
...
- (void)viewWillLayoutSubviews {
    self.menuButton.frame = CGRectMake(10, self.view.bounds.size.height - 95, self.menuButton.frame.size.width, self.menuButton.frame.size.height);
...
//    MDViewController *viewController = [self.viewControllers objectAtIndex:0];
//    [self.view bringSubviewToFront:viewController.buttonsView];
}
...
//    }
    ...
    openedPosition.origin = CGPointMake(50, -50);
...
//            self.contentView.frame = CGRectOffset(openedPosition, -10, 10);
        ...
        completion:^(BOOL finished) {
            self.isAnimating = NO;
            MDViewController *viewController = [self.viewControllers objectAtIndex:0];
            for (UIButton *button in viewController.buttonsView.subviews) {
...
- (void)closeMenu {
    ...
    for (id button in self.view.subviews) {
        if ([button isKindOfClass:[UIButton class]]) {
            [button removeFromSuperview];
            [viewController.buttonsView addSubview:button];
        }
